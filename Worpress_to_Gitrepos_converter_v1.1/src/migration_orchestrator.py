"""
Migration Orchestrator - Enhanced Version 1.1
Coordinates the entire WordPress to Git migration process
New features: Categories, Tags, Plugin documentation
"""
...
from .site_documenter import SiteDocumenter

class MigrationOrchestrator:
    """Orchestrates the complete migration process - Enhanced Version"""
    ...
    def __init__(self, config: ConfigManager):
        ...
        self.documenter: Optional[SiteDocumenter] = None
    ...
    def _load_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'migrated_pages': [],
            'migrated_media': [],
            'documented_categories': False,
            'documented_tags': False,
            'documented_plugins': False
        }
    ...
    def document_site(self, output_dir: Path) -> Dict[str, int]:
        """
        Document WordPress site configuration (NEW in v1.1)
        
        Returns:
            Dictionary with counts of documented items
        """
        logger.info("Starting site documentation...")
        
        # Initialize documenter
        self.documenter = SiteDocumenter(output_dir)
        ...
        try:
            ...
            if categories:
                self.documenter.document_categories(categories)
                stats['categories'] = len(categories)
                self.migration_state['documented_categories'] = True
        except Exception as e:
            logger.error(f"Failed to document categories: {e}")
        
        # Document tags
        try:
            tags = self.wp_client.get_tags()
            if tags:
                self.documenter.document_tags(tags)
                stats['tags'] = len(tags)
                self.migration_state['documented_tags'] = True
        except Exception as e:
            logger.error(f"Failed to document tags: {e}")
        
        # Document plugins
        try:
            plugins = self.wp_client.get_plugins()
            if plugins:
                self.documenter.document_plugins(plugins)
                stats['plugins'] = len(plugins)
                self.migration_state['documented_plugins'] = True
        except Exception as e:
            logger.warning(f"Failed to document plugins: {e}")
        
        # Document site info and theme
        try:
            site_info = self.wp_client.get_site_info()
            theme_info = self.wp_client.get_theme_info()
            self.documenter.document_site_info(site_info, theme_info)
        except Exception as e:
            logger.error(f"Failed to document site info: {e}")
        
        logger.info("Site documentation completed")
        return stats
    ...
    def migrate(self, dry_run: bool = False) -> None:
        ...
        """
        logger.info("=" * 60)
        logger.info("WordPress to Git Migration Started (v1.1)")
        logger.info("=" * 60)
        ...
        pages_metadata = []
        media_count = 0
        doc_stats = {'categories': 0, 'tags': 0, 'plugins': 0}
        ...
        # Document site (NEW in v1.1)
        doc_stats = self.document_site(output_dir)
        
        # Create index page
        if posts_metadata:
            self.create_index(output_dir, posts_metadata)
        
        # Create migration summary (NEW in v1.1)
        if self.documenter:
            summary_stats = {
                'posts': len(posts_metadata),
                'pages': len(pages_metadata),
                'media': media_count,
                'categories': doc_stats['categories'],
                'tags': doc_stats['tags'],
                'plugins': doc_stats['plugins'],
                'repository': self.github_manager.get_repository_url()
            }
            self.documenter.create_migration_summary(summary_stats)
        ...
        # Print summary
        logger.info("=" * 60)
        logger.info("Migration Summary (v1.1):")
        ...
        logger.info(f"  Categories documented: {doc_stats['categories']}")
        logger.info(f"  Tags documented: {doc_stats['tags']}")
        logger.info(f"  Plugins documented: {doc_stats['plugins']}")
