...
async function requireSub(req: any, res: any, next: any) {
  ...
  // Founding members have permanent access — no subscription needed
...
async function initDB() {
  try {
    ...
    // Add founding member columns if they don't exist (migration for existing DBs)
    await pool.query(`
      DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='users' AND column_name='is_founding_member') THEN
          ALTER TABLE users ADD COLUMN is_founding_member BOOLEAN DEFAULT false;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='users' AND column_name='founding_member_number') THEN
          ALTER TABLE users ADD COLUMN founding_member_number INTEGER;
        END IF;
      END $$;
    `);

    console.log('Database initialized with listings model + founding member support');
...
app.post('/auth/register', async (req, res) => {
  try {
    ...
    // Check if this user qualifies as a founding member (first 25 users)
    ...
    const user = rows[0];
    if (isFoundingMember) {
      console.log(`🌟 Founding Member #${foundingMemberNumber} registered: ${user.email}`);
    }
...
app.post('/auth/login', async (req, res) => {
  try {
    ...
    const {rows} = await pool.query('SELECT id,email,display_name,password_hash,salt,subscription_status,subscription_provider,is_admin,is_founding_member,founding_member_number FROM users WHERE email=$1',[email.toLowerCase()]);
    if (rows.length === 0) return res.status(401).json({error:'Invalid credentials'});
    const u = rows[0];
    if (hashPw(password, u.salt) !== u.password_hash) return res.status(401).json({error:'Invalid credentials'});
    res.json({ user:{id:u.id,email:u.email,display_name:u.display_name,subscription_status:u.subscription_status,is_admin:u.is_admin,is_founding_member:u.is_founding_member,founding_member_number:u.founding_member_number}, token:mkToken(u.id,u.email) });
  } catch { res.status(500).json({error:'Login failed'}); }
});

app.get('/auth/me', requireAuth, async (req: any, res) => {
  try {
    const {rows} = await pool.query('SELECT id,email,display_name,subscription_status,subscription_provider,subscription_expires_at,is_admin,is_founding_member,founding_member_number,created_at FROM users WHERE id=$1',[req.user.id]);
...
app.get('/admin/stats', requireAdmin, async (_req, res) => {
  try {
    ...
    const founderLimit = FOUNDING_MEMBER_LIMIT;
    res.json({
      total_users:parseInt(users.rows[0].count),
      total_listings:parseInt(listings.rows[0].count),
      pending_listings:parseInt(pending.rows[0].count),
      active_subscribers:parseInt(active.rows[0].count),
      pending_reports:parseInt(reports.rows[0].count),
      founding_members:parseInt(founders.rows[0].count),
      founding_member_limit:founderLimit,
    });
  } catch { res.status(500).json({error:'Failed'}); }
});
...
app.get('/admin/users', requireAdmin, async (_req, res) => {
  try {
    const {rows} = await pool.query(`SELECT id, email, display_name, subscription_status, subscription_provider, is_admin, is_founding_member, founding_member_number, created_at FROM users ORDER BY id ASC`);
    res.json(rows);
  } catch { res.status(500).json({error:'Failed'}); }
});

// Admin: grant founding member status manually
app.post('/admin/users/:id/grant-founding', requireAdmin, async (req: any, res) => {
  try {
    const userId = parseInt(req.params.id);
    // Get next founding member number
    ...
    res.json({message:`User granted Founding Member #${nextNum} status with permanent access`});
  } catch { res.status(500).json({error:'Failed'}); }
});

// Admin: revoke founding member status
app.post('/admin/users/:id/revoke-founding', requireAdmin, async (req: any, res) => {
  try {
    const userId = parseInt(req.params.id);
    await pool.query('UPDATE users SET is_founding_member=false, founding_member_number=null, subscription_status=$1 WHERE id=$2',['none', userId]);
    res.json({message:'Founding member status revoked'});
  } catch { res.status(500).json({error:'Failed'}); }
});
...
// Admin: toggle admin via admin dashboard (requires existing admin auth)
app.post('/admin/users/:id/toggle-admin', requireAdmin, async (req: any, res) => {
  try {
    ...
    await pool.query('UPDATE users SET is_admin=$1 WHERE id=$2',[newStatus, userId]);
    res.json({message: newStatus ? 'User promoted to admin' : 'Admin access removed', is_admin: newStatus});
  } catch { res.status(500).json({error:'Failed'}); }
});
...
app.post('/admin/users/:id/suspend', requireAdmin, async (req: any, res) => {
  try {
    const userId = parseInt(req.params.id);
    if (userId === req.user.id) return res.status(400).json({error:'Cannot suspend yourself'});
    await pool.query("UPDATE users SET subscription_status='suspended' WHERE id=$1",[userId]);
    res.json({message:'User suspended'});
  } catch { res.status(500).json({error:'Failed'}); }
});

// Admin: activate user subscription manually
app.post('/admin/users/:id/activate', requireAdmin, async (req: any, res) => {
  try {
    const userId = parseInt(req.params.id);
    const exp = new Date(); exp.setMonth(exp.getMonth()+1);
    await pool.query('UPDATE users SET subscription_status=$1,subscription_provider=$2,subscription_expires_at=$3 WHERE id=$4',['active','admin',exp.toISOString(),userId]);
    res.json({message:'User subscription activated for 1 month'});
  } catch { res.status(500).json({error:'Failed'}); }
});
